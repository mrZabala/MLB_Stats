...
<script>
    new Sortable(document.querySelector('#tabla-jugadores tbody'), {
    animation: 150,
...
$(document).ready(function() {
    ...
    // Hacer que las filas sean arrastrables con jQuery UI
    $('#tabla-jugadores tbody').sortable({
        items: 'tr', // Solo las filas serán arrastrables
        cursor: 'move', // Cambiar el cursor al arrastrar
        update: function(event, ui) {
            // Acción después de mover la fila
            var order = table.rows().nodes().toArray().map(function(row) {
                return $(row).find('td').first().text(); // Obtener el nombre del jugador
            });
